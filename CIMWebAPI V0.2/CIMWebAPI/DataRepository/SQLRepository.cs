...
namespace CIMWebAPI.DataRepository
{
    ...
    {
        public bool PresenceDuplicate(int SourceID, int LoadID, int ServiceID, string connString)
        {
        ...
        public async Task<OutQueuePhone> QueryBatchService(OutQueuePhone InData)
        {
            try
            {
                DataTable result = new DataTable();
                using (var conn = new SqlConnection(InData.connStringCIM))
                {
                    ...
                    {
                        conn.Open();
                        cmd.CommandText = @"SELECT COUNT(*) AS CountBatch FROM APITable WHERE CONVERT(DATE, Received) = CONVERT(DATE, GETDATE()) AND Batch = @BatchNumber AND ServiceID = @ServiceID";
                        cmd.CommandTimeout = 0;
                        cmd.Parameters.AddWithValue("@BatchNumber", InData.BatchNumber);
                        cmd.Parameters.AddWithValue("@ServiceID", InData.ServiceID);
                        SqlDataReader reader = cmd.ExecuteReader(CommandBehavior.CloseConnection);
                        result.Load(reader);
                    }
                ...
                {
                    InData = await GetLastLoadID(InData);
                }
            ...
            try
            {
                using (var conn = new SqlConnection(InData.connStringPresence))
                {
                    using (SqlCommand cmd = conn.CreateCommand())
                    {
                        ...
                        cmd.Parameters.AddWithValue("@ServiceID", InData.ServiceID);
                        cmd.Parameters.AddWithValue("@LoadID", InData.LoadID);
                        if (InData.LoadDescription == String.Empty)
                        {
                            InData.LoadDescription = "CIMAPILoad" + DateTime.Now.Year.ToString() + DateTime.Now.Month.ToString() + DateTime.Now.Day.ToString() + " - Batch Number: " + InData.BatchNumber.ToString();
                        }
        ...
        public async Task<string> FormatNumber(string inNumber, string connString)
        {
            try
            {
                string query = @"SELECT [dbo].[CIMNumber] (@NumberIn) AS ResultOut";
        ...
        public async Task<bool> LogAPI(DataTable inTable, string connString)
        {
            using (SqlConnection conn = new SqlConnection(connString))
            {
                ...
                bulkCopy.DestinationTableName = "[dbo].[APILog]";
        ...
        public async Task<bool> logError(OutQueuePhone data, string Class, string method, string exception)
        {
            try
            {
                string query = @"INSERT INTO [dbo].[LogConsoleError]
                                (
                                    [InstanceID]
                                    ,[Class]
                                    ,[Method]
        ...
        public async Task<OutQueuePhone> GetLastLoadID(OutQueuePhone InData)
        {
            try
            {
                DataTable result = new DataTable();
                using (var conn = new SqlConnection(InData.connStringCIM))
                {
                    ...
                    {
                        conn.Open();
                        cmd.CommandText = "SELECT TOP 1 LoadID FROM APITable WHERE CONVERT(DATE, Received) = CONVERT(DATE,GETDATE()) AND Batch = @BatchNumber AND ServiceID = @ServiceID ORDER BY LOADID DESC";
                        cmd.CommandTimeout = 0;
                        cmd.Parameters.AddWithValue("@BatchNumber", InData.BatchNumber);
                        cmd.Parameters.AddWithValue("@ServiceID", InData.ServiceID);
                        SqlDataReader reader = cmd.ExecuteReader(CommandBehavior.CloseConnection);
