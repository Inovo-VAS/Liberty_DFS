        ...
        public async Task<IActionResult> Post([FromBody] OutQueuePhone data)
        {
            ...
            try
                ...
                if (IsAuthorized)
                {
                    ...
                    {
                        if (data.LoadID == 0 && data.BatchNumber == 0 && data.Command.ToLower() == "addcall")
                        {
                            APILogRow["ReturnMessage"] = "Bad Request Status 400 - No Load ID or Batch Number provided.";
                            APILogRow["ReturnTime"] = DateTime.Now;
                            APILog.Rows.Add(APILogRow);
                            await sqlRepo.LogAPI(APILog, data.connStringCIM);
                        ...
                        else if (data.Phone == "" && data.Phone1 == "")
                        {
                        ...
                        try
                        {
                            if (data.Command.ToLower() == "addcall")
                            {
                                ...
                                try
                                {
                                    if(data.BatchNumber != 0 && data.LoadID == 0)
                                    {
                                        data = await data.GetBatch();
                                    }
                                }
                                catch (Exception ex)
                                {
                            ...
                            {
                                sqlBulk.DestinationTableName = "[dbo].[QueuePhoneComplete]";
                                sqlBulk.BatchSize = 1;
                            ...
                            using (SqlBulkCopy sqlBulk_ = new SqlBulkCopy(ConnStringCIM))
                            {
                                if (isDev)
                                {
                                    sqlBulk_.DestinationTableName = "[dbo].[APITableDev]";
                                }
                                else
                                {
                                    sqlBulk_.DestinationTableName = "[dbo].[APITable]";
                                }
